...
test('service worker installs the parsed v97 application shell with pricing and generation planning before app.js', async () => {
  ...
  assert.match(sw, /const CACHE_VERSION\s*=\s*'v97';/);
  for (const asset of ['./css/manuscript-workspace.css', './js/manuscript-workspace.js']) {
    assert.ok(appShell.includes(asset));
    assert.ok(fs.existsSync(path.join(__dirname, '..', 'public', asset)));
  }
  assert.ok(appShell.includes('./css/bookshelf-atelier.css'));
  assert.ok(appShell.indexOf('./js/book-design.js') >= 0);
  assert.ok(appShell.indexOf('./js/book-design.js') < appShell.indexOf('./js/app.js'));
