...
test('尖峰時段每天包含 UTC 01:00–04:00 與 06:00–10:00，週末不例外', () => {
  ...
  assert.equal(isPeakTime(new Date('2026-09-05T02:00:00Z')), true);
  assert.equal(isPeakTime(new Date('2026-09-06T08:00:00Z')), true);
});
...
test('V4 Flash 依尖離峰及快取命中比例計價', () => {
  ...
  assert.equal(calculateUsageCost(usage, 'deepseek-v4-flash', new Date('2026-09-05T04:00:00Z')), 0.82675);
  assert.equal(calculateUsageCost(usage, 'deepseek-v4-flash', new Date('2026-09-05T02:00:00Z')), 1.6535);
...
test('舊回應沒有快取明細時將全部輸入視為 cache miss', () => {
  ...
  assert.equal(calculateUsageCost(usage, 'deepseek-v4-flash', new Date('2026-09-05T04:00:00Z')), 0.88);
});
