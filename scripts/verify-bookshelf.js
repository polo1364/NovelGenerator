...
async function run() {
  ...
  try {
    ...
    if (!baseline) {
      ...
      await poll('offline shell cached',()=>client.evaluate(`caches.open('novel-workshop-v93').then(async c => !!(await c.match('./js/book-design.js')) && !!(await c.match('./css/bookshelf-atelier.css')))`));
