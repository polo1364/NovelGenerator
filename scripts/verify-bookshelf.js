...
async function run() {
  ...
  try {
    ...
    if (!baseline) {
      ...
      await poll('offline shell cached',()=>client.evaluate(`caches.open('novel-workshop-v99').then(async c => !!(await c.match('./js/book-design.js')) && !!(await c.match('./css/bookshelf-atelier.css')) && !!(await c.match('./css/reader-modes.css')) && !!(await c.match('./assets/sakura.webp')) && !!(await c.match('./js/manuscript-workspace.js')) && !!(await c.match('./css/manuscript-workspace.css')) && !!(await c.match('./css/mobile-touch.css')))`));
