...
// Node 21+ 內建 WebSocket；舊版 Node 改用 ws 套件
const WebSocketImpl = (() => {
...
function buildSsml(text, { voice, style, rate, pitch }) {
  // Edge readaloud WS 對 <phoneme> 回 1007（不支援），
  // 改用發音安全改寫（繁→簡多音字）+ word joiner 保護詞彙
  const prepared = applyWordJoiners(applyPolyphoneHints(stripPhonemeTags(text)));
  const escaped = escapeXmlPreservingPhoneme(prepared);
...
async function synthesizeViaProxy(proxyUrl, text, options) {
  ...
  try {
    const response = await fetch(proxyUrl.replace(/\/$/, ''), {
      ...
      body: JSON.stringify({
        // 代理 Worker 不支援 SSML（phoneme 標籤會被唸出來），
        // 改用發音安全改寫（繁→簡多音字）+ word joiner 保護詞彙
        text: applyWordJoiners(applyPolyphoneHints(stripPhonemeTags(text))),
        voice: options.voice,
        style: options.style,
        rate: options.rate,
        pitch: options.pitch
...
function synthesizeViaWebSocket(text, options, retry = true) {
  ...
  if (!WebSocketImpl) {
    return Promise.reject(new Error('此 Node 版本無 WebSocket，請升級 Node 21+ 或安裝 ws 套件'));
  }

  return new Promise((resolve, reject) => {
    const audioParts = [];
    let settled = false;
    const ws = new WebSocketImpl(wsUrl, { headers: EDGE_WS_HEADERS });
    ...
    const timeout = setTimeout(() => finish(new Error('Edge TTS 連線逾時')), 30000);
    // 連線階段獨立逾時，避免握手掛住拖慢退回代理
    const connectTimeout = setTimeout(() => finish(new Error('Edge TTS 連線逾時（握手）')), 8000);
...
// WebSocket 連續失敗時暫停直連一段時間，避免每段朗讀都等逾時
let wsCooldownUntil = 0;
const WS_FAIL_COOLDOWN_MS = 5 * 60 * 1000;
...
async function synthesizeEdgeTts(text, options = {}) {
  ...
  // WebSocket 直連支援 SSML phoneme（多音字讀音最準），優先使用；失敗才退回代理
  if (WebSocketImpl && Date.now() >= wsCooldownUntil) {
    try {
      return await synthesizeViaWebSocket(text, opts);
    } catch (err) {
      wsCooldownUntil = Date.now() + WS_FAIL_COOLDOWN_MS;
      console.warn('[edge-tts] WebSocket 直連失敗，改走代理:', err.message);
    }
  }
  if (proxyUrl && proxyUrl.toLowerCase() !== 'off') {
    return synthesizeViaProxy(proxyUrl, text, opts);
  }
  throw new Error('Edge TTS 無可用合成路徑（WebSocket 失敗且代理已停用）');
}
